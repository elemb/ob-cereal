...
ob-cereal lets you talk to (probably) any serial device directly via org-mode code blocks.

You write commands in 
=#+BEGIN_SRC cereal= blocks → 
→ Org sends them over real serial →
→ you get the reply as 
→ =#+RESULTS:=.

Perfect for:
- Literate embedded development / firmware REPL sessions
- Documenting hardware experiments
- Talking to Forth systems (zeptoForth, Mecrisp-Stellaris, FlashForth, ESP32Forth, etc.)
- I²CDriver (https://github.com/jamesbowman/i2cdriver
- SPIDriver https://github.com/jamesbowman/spidriver
- TermDriver2 https://github.com/jamesbowman/termdriver2
- Bus Pirate, FTDI adapters, etc.

* Status 2026

The original repo (https://github.com/andrmuel/ob-uart) seems abandoned for many years.

This fork (renamed to ob-cereal) contains fixes that help it work in modern Emacs / Doom Emacs:
...
RESULTS:
: : STAR ( -- ) 42 EMIT ;  ok
: : STARS ( n -- ) 0 DO STAR SPACE LOOP ;  ok
: 7 stars * * * * * * *  ok



* Recommended header args (Mac)

| Argument      | Typical value              | Meaning / Notes                                 |
|---------------|----------------------------|-------------------------------------------------|
| :port         | /dev/cu.usbmodem*          | Required. Use quotes if name has spaces         |
