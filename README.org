#+TITLE: ob-cereal – Literate UART / serial communication in Org-mode
#+AUTHOR: (your name / original Andreas Müller + elemb + your improvements)
#+LANGUAGE: en

* What is this?

ob-uart lets you talk to microcontrollers, sensors, Bus Pirate, or any serial device directly from Org-mode code blocks.

You write commands (Forth, AT commands, whatever) in =#+BEGIN_SRC uart= blocks → Org sends them over real UART → you get the reply as =#+RESULTS:=.
...
This fork contains **critical fixes** that make it actually work in modern Emacs / Doom Emacs:
...
If you find this useful, feel free to fork again and rename to =ob-cereal= or similar.

* Installation

** Doom Emacs (recommended – this is how it was actually tested)

1. In your =~/.doom.d/packages.el= (or =~/doom.d/packages.el=), add:

   #+begin_src emacs-lisp
   (package! ob-uart
     :recipe (:host github :repo "YOURUSERNAME/ob-uart" :files ("ob-uart.el")))
   #+end_src
...
2. Run in terminal:
   ...
   #+begin_src sh
   doom sync
   doom build    # optional but recommended after first install
   #+end_src
...
6. Verify:
   ...
   - =M-x eval-expression RET (featurep 'ob-uart) RET= → should return =t=
   - =M-x eval-expression RET (fboundp 'org-babel-execute:uart) RET= → =t=
...
#+begin_src emacs-lisp
(add-to-list 'load-path "/path/to/your/ob-uart-folder")
(require 'ob-uart)
(org-babel-do-load-languages
 'org-babel-load-languages
 '((uart . t)))
...
#+BEGIN_SRC uart :port /dev/cu.usbmodem31401 :speed 115200 :bytesize 8 :parity none :stopbits 1 :flowcontrol none
armv6m words-in
#+END_SRC
...
#+BEGIN_SRC uart :port /dev/cu.usbmodem31401 :speed 115200 :parity none :flowcontrol none
decimal 1 .
#+END_SRC
...
- **Still getting =wrong-type-argument: stringp, 115200= ?**
  ...
  - Fix: =rm -rf ~/.emacs.d/.local/straight/build-*/ob-uart= then =doom sync= again + full Emacs restart.

- **No output or garbage (^F ^M etc.)**
  - The current version strips most control chars automatically.
  - If you still see junk → enable debug: =(setq ob-uart-debug t)= and look at =*Messages*=.
...
* Why literate Forth + UART is powerful
